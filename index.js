...
app.get('/', function (req, res) {
    ...
    service.findAddressCandidates({
        ...
        webmap = options.webmap || getDefaultWebmap(xy, extentValue);
        ...
        if(options.markers){
            ...
            for(i=0;i<options.markers.length; i++){
                ...
                latlong = m[1].split(",");
                obj = {
                    color: m[0].substr(6,m[0].length-1),
        ...
        if(options.maptype){options.basemap = options.maptype;}
...
var getDefaultWebmap = function(xy, extentValue){
    return {
        "mapOptions": {
